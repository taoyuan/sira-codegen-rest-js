...
module.exports = function (opts, type) {
    var classes = toRestClasses(opts.classes);
    return {
        isNode: type === 'node',
        url: opts.url,
...
function toRestClasses(classes) {
    if (!Array.isArray(classes)) return [];
    if (classes.length === 0) return classes;
    if (classes[0] instanceof rest.RestClass) return classes;
    return rest.Rest.buildClasses(classes);
}
